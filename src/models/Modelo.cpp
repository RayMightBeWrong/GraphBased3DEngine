...
void Modelo::writeV(std::ofstream& myFile, float x, float y, float z,float nx,float ny,float nz,float tx,float ty){
	myFile << x; myFile << " ";
	myFile << y; myFile << " ";
	myFile << z; myFile << " ";
	myFile << nx; myFile << " ";
...
void Modelo::writeFile(std::ofstream& myFile, 
	...
	for(int i = 0; i < verticeCount; i++)
		writeV(myFile, vertexs[i*3], vertexs[i*3+1], vertexs[i*3+2] , normals[i*3],normals[i*3+1],normals[i*3+2],textures[i*2],textures[i*2 + 1]);
